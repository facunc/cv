...
async function consulta(){
    let datosPersonales = await fetch("https://randomuser.me/api/")
    ...
    document.getElementById('nombre').innerHTML = dato.name.first + " " + dato.name.last;
    document.getElementById('origen').innerHTML = dato.nat;
    document.getElementById('edad').innerHTML = dato.dob.age;
    document.getElementById('residencia').innerHTML = dato.location.street.number +' '+ dato.location.street.name + ', ' + dato.location.city + ', ' + dato.location.country + ' ' + '(' + dato.location.postcode + ')'; 
}
